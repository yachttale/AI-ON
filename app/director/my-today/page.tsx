...
export default function DirectorMyTodayPage() {
  ...
  async function load() {
    ...
    setStudents((allStudents ?? []).filter(s => getTodayEntries(s.schedule).length > 0))
    setLogs(todayLogs ?? [])
    setLoading(false)
  }
  ...
  async function setAttendance(studentId: string, attendance: Attendance) {
    ...
    if (existing) {
      ...
      await supabase.from('session_logs').insert({ student_id: studentId, instructor_id: user.id, session_date: today, attendance })
    }
    await load()
  }
  ...
  if (loading) return <div className="space-y-3">{[...Array(3)].map((_, i) => <div key={i} className="h-24 bg-gray-200 rounded-xl animate-pulse" />)}</div>

  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-lg font-bold text-gray-800">내 수업 (오늘)</h2>
          ...
          {students.map(s => {
            ...
            return (
              <div key={s.id} className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
                <Link href={`/director/student/${s.id}`} className="flex items-center justify-between px-4 pt-3 pb-2">
                  <div>
                ...
                <div className="flex gap-1.5 px-3 pb-3">
                  {(['출석', '지각', '결석'] as Attendance[]).map(att => (
                    <button key={att} onClick={() => setAttendance(s.id, att)}
                      className={`flex-1 py-2 rounded-lg text-xs font-semibold transition-all ${log?.attendance === att ? ATTENDANCE_COLORS[att] : 'bg-gray-100 text-gray-400'}`}>
                      {att}
                    </button>
                  ))}
                </div>
