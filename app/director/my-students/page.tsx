...
export default async function DirectorMyStudentsPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <div className="grid grid-cols-3 gap-2">
          {strokeGroups.map(({ stroke, count }) => (
            <div key={stroke} className="bg-white rounded-xl p-3 border border-gray-100 shadow-sm text-center">
              <p className="text-xs text-gray-500 mb-1">{stroke}</p>
              <p className="text-xl font-bold text-gray-800">
                {count}<span className="text-xs font-normal text-gray-400 ml-0.5">명</span>
              </p>
            </div>
          ))}
          {noStrokeCount > 0 && (
            <div className="bg-gray-50 rounded-xl p-3 border border-gray-100 text-center">
              ...
              <p className="text-xl font-bold text-gray-400">
                {noStrokeCount}<span className="text-xs font-normal ml-0.5">명</span>
              </p>
            </div>
          )}
        </div>
      ...
      <div>
        ...
        <StudentProgressList students={studentProgress} basePath="/director/student" />
      </div>
    </div>
  )
}
